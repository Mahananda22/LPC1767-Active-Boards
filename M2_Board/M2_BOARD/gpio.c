...
void Init_M2Board_GPIO_Pins(void) {
    // Input Pins
    GPIO_InitPin(2, 5, 0, 0);
    GPIO_InitPin(2, 13, 0, 0);
    GPIO_InitPin(1, 24, 0, 0);
    GPIO_InitPin(1, 22, 0, 0);
    GPIO_InitPin(1, 21, 0, 0);
	  GPIO_InitPin(0, 26, 0, 0);
	  GPIO_InitPin(0, 11, 0, 0);
   // GPIO_InitPin(1, 31, 0, 0);
    // Output Pins
    GPIO_InitPin(2, 12, 1, 0);
    GPIO_InitPin(2, 11, 1, 0);
    GPIO_InitPin(0, 27, 0, 0);//cn19 pin 1
    GPIO_InitPin(1, 31, 0, 0);//CN19 PIN 2
    GPIO_InitPin(3, 26, 1, 0);
    GPIO_InitPin(3, 25, 1, 0);
	  GPIO_InitPin(0, 10, 1, 0);
	  GPIO_InitPin(0, 0, 1, 0);
}
