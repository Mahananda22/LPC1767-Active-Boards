...
case CMD_WRITE_GPIO:
    if (cmd_buffer[1] == 0xF3 || cmd_buffer[1] == 0xF4) {
        // DAC write command: [CMD_WRITE_GPIO, F3/F4, channel, MSB, LSB]
        if (buffer_index != 5) {
            SendNak(0xE3);  // Invalid length
            return;
        }

        uint8_t dac_id      = cmd_buffer[1]; // F3 or F4
        uint8_t dac_channel = cmd_buffer[2];
        uint16_t dac_value  = (cmd_buffer[3] << 8) | cmd_buffer[4];
        ...
        if (dac_id == 0xF3) {
            TLV5618_U16(dac_channel, dac_value); // DAC1
        } else {
            TLV5618_U27(dac_channel, dac_value); // DAC2
        }

        SendAck(CMD_WRITE_GPIO);
    }
    else {
        // GPIO write command: [CMD_WRITE_GPIO, port, pin, val]
        if (buffer_index != 4) {
            SendNak(0xE3);  // Invalid length
            return;
        }
